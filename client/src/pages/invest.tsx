...
function ProductCard({ product, userInvestments, investMutation, buyingProductId, hasActiveFixed, maxVip, openConfirmProduct }: any) {
  const notYetLaunched = product.launchDate && new Date(product.launchDate) > new Date();
  const countdown = useCountdown(notYetLaunched ? product.launchDate : null);
  ...
  return (
    <div data-testid={`product-card-${product.id}`}
      ...
      <div className="px-4 py-3">
        {!isLaunched ? (
          <div className="w-full rounded-2xl overflow-hidden border border-amber-100 bg-amber-50">
            <div className="flex items-center justify-center gap-1.5 px-3 py-2 border-b border-amber-100">
              <Calendar className="w-3.5 h-3.5 text-amber-500 shrink-0" />
              <span className="text-amber-700 text-xs font-semibold">
                Disponible le {new Date(product.launchDate).toLocaleDateString("fr-FR", { day: "2-digit", month: "2-digit", year: "numeric" })} à {new Date(product.launchDate).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}
              </span>
            </div>
            {countdown && (
              <div className="flex items-center justify-center py-2">
                <span className="text-amber-600 font-black text-sm tracking-widest">{countdown}</span>
              </div>
            )}
          </div>
        ) : !hasActiveFixed ? (
...
export default function InvestPage() {
  ...
  const { data: adminProducts = [], isLoading: loadingProducts } = useQuery<any[]>({ queryKey: ["/api/products"], refetchInterval: 30000 });
        ...
        <div className="flex bg-white rounded-2xl p-1 shadow-sm border border-gray-100">
          {([
            ...
            <button key={t.key} data-testid={`tab-${t.key}`}
              ...
              className={`flex-1 py-2.5 rounded-xl text-sm transition-all ${activeTab === t.key ? "text-black shadow" : "text-gray-400"}`}
          ...
          {fixedPlan.plans.map((plan) => (
            <div key={plan.vip} data-testid={`plan-card-${plan.vip}`}
              ...
              <div className="relative h-36 overflow-hidden">
                <img src="/sinopec-logo.jpeg" alt={plan.name} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute top-2 left-2 text-white text-[10px] font-bold px-2.5 py-1 rounded-full"
                  style={{ background: "linear-gradient(135deg, #F59E0B, #D97706)" }}>
                  90 jours
        ...
        <div className="px-4 space-y-3">
          ...
          {!loadingProducts && availableProducts.map((product: any) => (
            <ProductCard
              ...
              investMutation={investMutation}
              buyingProductId={buyingProductId}
              hasActiveFixed={hasActiveFixed}
              maxVip={maxVip}
              openConfirmProduct={openConfirmProduct}
            />
          ))}
        </div>
      )}
