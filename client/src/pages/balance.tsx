...
export default function BalancePage() {
  ...
  const items = [
    { label: "Solde Total", value: totalBalance, icon: Wallet, color: "text-blue-500", bg: "bg-blue-50", note: "" },
    { label: "Solde de Recharge", value: user.depositBalance, icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50", note: "Pour acheter des produits" },
    { label: "Solde de Retrait", value: user.withdrawBalance, icon: DollarSign, color: "text-amber-500", bg: "bg-amber-50", note: "Gains, commissions, spin" },
    { label: "Revenu Produit", value: user.productRevenue, icon: ShoppingBag, color: "text-purple-500", bg: "bg-purple-50", note: "" },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header gold */}
      <div style={{ background: "linear-gradient(135deg, #F59E0B 0%, #D97706 100%)" }} className="px-4 pt-6 pb-6">
      ...
      <div className="mx-4 mt-4 bg-white rounded-2xl shadow-sm border border-gray-100">
        {items.map((item, i) => (
          <div key={item.label} className={`flex items-center justify-between px-4 py-4 ${i < items.length - 1 ? "border-b border-gray-50" : ""}`}>
      ...
      <div className="mx-4 mt-3 bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        ...
        <div className="flex gap-2">
          <input
            className="flex-1 border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none bg-gray-50 uppercase font-mono placeholder-gray-300 focus:border-amber-400"
          ...
          />
          <button
            className="text-black text-sm font-bold px-4 py-2.5 rounded-xl disabled:opacity-60"
